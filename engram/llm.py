...
_ANSWER_SYS = (
    ...
    "- The SAME real-world attribute may appear under DIFFERENT predicate names "
    "(e.g. 'charity_5k_personal_best_time' and 'personal_best_5k_time' are the same "
    "thing). Treat every fact relevant to the question as a candidate for one "
    "answer. When candidates give different values, you MUST return the one with "
    "the most recent 'when' date — never an older value when a newer relevant fact "
    "exists. A fact marked status=current also wins over status=superseded.\n"
